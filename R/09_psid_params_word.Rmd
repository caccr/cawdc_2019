...
l1d <- " **average chemical findings exceeded their MCL, though exceedance alone does not constitute a violation (see the FAQ for more information)**. <br>"
...
l6a <- "The following **" 
l6b <- "** water quality indicators were tested for in your system and, may have been detected (see the table below), though they pose no serious human health hazard, and hence have an MCL of 0: "  
...
l7 <- " other chemicals were tested for and NOT detected. They are not included in the chart above, but are reported in the table below."
l8 <- "** water quality indicators were tested for and may have been detected, though they pose no serious human health hazard (MCL = 0). They are reported in the table below."
...
`r paste0("**View an interactive version of this report online at**: caccr.github.io/ccrs/", params$psid, ".html")`
...
if(nrow(dg1) > 0){
  p <- mutate(dg1) %>% 
    #, CHEMICAL__ = stringr::str_wrap(CHEMICAL__, 15,1,1)) %>% 
    ...
    coord_cartesian() +
    labs(x = NULL, y = "Percent MCL Exceedance (%)", fill = "% Exceedance",
...
Explore your water quality data in the table below.
...
dg %>% 
  select(CHEMICAL__, mean_finding, MCL, RPT_UNIT, XMOD) %>% 
  mutate(mean_finding = round(mean_finding, 2)) %>% 
  rename(CHEMICAL = CHEMICAL__,
         SAMPLE_MEAN = mean_finding, 
         UNIT = RPT_UNIT) %>% 
  flextable() %>% 
  autofit()
          #     "latex",
          #     booktabs = TRUE, 
          #     longtable= TRUE,
          # col.names = c("CHEMICAL", "SAMPLE_MEAN","MCL","UNIT", "XMOD")) #%>%
...
- services **`r tsc`** connections to serve a total population of **`r pop`** individuals.  
- has a primary water source type of **`r wst`**.  
...
> `r paste0("Health Agency: ", cd$HEALTH_DISTRICT)`   
> `r cd$HD_ADDRESS`   
> `r cd$HD_PHONE`    
> `r if(!is.na(cd$HD_EMAIL)) cd$HD_EMAIL`  
...
To view frequently asked questions, visit the FAQ online: [caccr.github.io/faq/](https://caccr.github.io/faq/)  
